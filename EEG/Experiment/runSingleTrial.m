function [resMat,relpos,altpos] = runSingleTrial(scr, const, expDes, my_key, t)
% ----------------------------------------------------------------------
...
% Send trigger to parallel port, then reset the port
outp(const.address, data_outP);
WaitSecs(0.01);
outp(const.address, 0);
...
% Otherwise save respective answer (corrisponding or not-corrisponding)     
    ...
    if resMat(1) == 0 && expDes.expMat(t,2) == 0
        rcode = 201;
    elseif resMat(1) == 0 && expDes.expMat(t,2) == 1
        rcode = 202;
    elseif resMat(1) == 1 && expDes.expMat(t,2) == 0
        rcode = 203;
